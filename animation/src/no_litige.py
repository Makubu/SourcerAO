...
class NoLitigation(MovingCameraScene):
    ...
    def no_litigation(self):
        ...
        text = self.get_text("Case 1: No Litigation", font_size=77)
        text.set_color_by_gradient(BLUE, RED)
        self.play(Write(text))
        footer = self.get_text("Case 1: No Litigation", font_size=20).to_corner(DL)
    ...
    def accept_project(self):
        ...
        index_agree = [0,1,7,9,10,2,4]
        index_disagree = [3,18,5]
        ...
        self.play(*[
            FadeOut(disagrees),
            agrees.animate.next_to(self.business, RIGHT),
            FadeOut(self.source_code, self.contract)
        ])
        self.play(FadeOut(agrees))
    ...
    def share_bounty(self):
        ...
        text = self.get_text("Bail redistribution", font_size=30).to_edge(DOWN)
        self.play(*[bail.animate.center(), Write(text)])
        self.play(
            *[
                bail[1].animate.next_to(self.business, RIGHT),
        ...
        new_text = self.get_text("Bounty distribution", font_size=30).to_edge(DOWN)
        self.play(*[bounty.animate.center(), Transform(text, new_text)])
        self.play(
            *[
                bounty[1].animate.next_to(bail[2], LEFT),
                FadeOut(bounty[0])
            ]
        )
        self.play(*[
            FadeOut(bail[1], bounty[1], bail[2], text)
        ])
